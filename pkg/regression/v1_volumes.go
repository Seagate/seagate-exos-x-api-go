...
var _ = DescribeRegression("Volume Testing (v1)", func(tc *TestContext) {
	...
	Describe("v1VolumeTest", func() {
		It("should successfully create the volume", func() {
			...
			logger.V(3).Info("CreateVolume", "name", volname1, "size", size, "wwn", volume.Wwn, "status", status.ResponseTypeNumeric)
		...
		It("should be able to expand the volume", func() {
			...
			logger.V(3).Info("ExpandVolume", "name", volname1, "response", status.ResponseTypeNumeric)
			Expect(err).To(BeNil())
			Expect(status.ResponseTypeNumeric).To(Equal(0))
		...
		It("should successfully delete both volumes", func() {
			...
			logger.V(3).Info("delete volume", "name", volname2, "err", err)
			Expect(err).To(BeNil())
			Expect(status.ResponseTypeNumeric).To(Equal(0))
